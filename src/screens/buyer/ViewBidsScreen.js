...
const ViewBidsScreen = ({ navigation, route }) => {
  ...
  const handleConfirmHire = async () => {
    ...
    try {
      ...
      // No payment here. Accepting a bid only creates the booking (status
      // 'ongoing', payment_status 'unpaid') — the buyer pays when the seller
      // submits work and the buyer accepts it, choosing direct vs hold then.
      setBids(prev =>
        prev.map(b => ({
          ...b,
          hired: b.id === bidId,
